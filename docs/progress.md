...
- `推荐增量步骤`：
  - [ ] A09-1 新增 `GET /api/v1/rooms`，先返回当前会话列表。
  - [ ] A09-2 新增 `GET /api/v1/users`，先返回当前用户列表。
  - [ ] A09-3 新增 `GET /api/v1/messages?room_id=...` 或兼容参数形式，先返回单会话消息历史。
  - [ ] A09-4 WebUI 仍可继续用 bootstrap，等资源接口稳定后再逐步拆分初始化逻辑。
