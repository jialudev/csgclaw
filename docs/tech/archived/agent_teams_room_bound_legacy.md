...
本文基于当前 `csgclaw` 源码、`/Users/russellluo/Projects/labs/claude-code` 中 Claude Code agent teams 的源码分析，以及早期 manager-to-worker skill 派活实践，重新收敛一版更适合 `csgclaw` 的 Agent Teams 方案。
...
2. 支持比普通 skill 派活更稳定的串行、并行、依赖、审批和人工干预；
...
早期多 agent 协作主要依赖 manager-to-worker skill 派活：
...
5. 不再保留独立 tracker fallback，统一走 `agent-teams`；
...
当前最合适的方案不是完整照搬 Claude Code agent teams，也不是继续只靠 skill 派活，而是采用中间路线：
