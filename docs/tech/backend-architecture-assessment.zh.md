...
- [前端架构评估](web/architecture-assessment.zh.md)
...
4. Runtime 通过中立的 `ChannelBinding` 获取 Channel 配置。
...
Runtime 只消费 Binding，不再知道 Feishu 类型。正式实现中，Secret 应使用只读 Value Object；任意 Map 只适合作为过渡结构。
