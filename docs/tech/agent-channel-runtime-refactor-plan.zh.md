...
> - 文档状态：待评审方案
> - 代码基线：`fb3bb9ee`（`upstream/main`）
> - 范围：Agent、Participant、Channel、Runtime 的控制面、数据面与装配边界

关联文档：

- [后端架构评估](backend-architecture-assessment.zh.md)：定义模块化单体的整体分层、
  Application Bootstrap、API Contract 与迁移原则；本文是其中 Agent / Channel /
  Runtime 纵向切面的实施方案。
...
本文不建立一套平行于后端整体架构的新分层。各概念必须落入
[后端架构评估](backend-architecture-assessment.zh.md) 定义的 Adapter、Application、
Domain、Infrastructure 和 API Contract 职责中：

| 本方案概念 | 整体分层 | 目标职责 | 不应承担 |
| --- | --- | --- | --- |
| `ChannelSource` / `ChannelSink` | Adapter / Infrastructure | 对接 Channel SDK、转换入站和出站格式 | Turn 编排、Runtime 类型判断 |
| `ExecutionPlanner` / `BridgeCoordinator` | Application | 选择执行模式，组织 Binding、Turn、Work 与投递 | HTTP 解析、具体 Channel SDK、文件存储细节 |
| Participant / Agent / Binding 规则 | Domain | 维护身份、绑定和合法性约束 | Credential 解析、进程启动 |
| Runtime Descriptor / Executor / Reconciler | Infrastructure Port | 暴露 Runtime 能力和执行入口 | Feishu 等具体 Channel 类型 |
| Event Envelope / Capability DTO | API Contract | 定义 extension 等跨进程调用方看到的稳定协议 | 保存运行状态、承载领域规则 |
| Session、去重、映射等 Store 实现 | Infrastructure | 按 Application Port 持久化运行状态 | 被 `internal/api` 直接持有或操作 |
...
Channel / HTTP Adapter
  -> Application orchestration
     -> Domain rules + Runtime / Store ports
        -> Channel SDK / Runtime / filesystem implementations
```

因此，`BridgeCoordinator` 即使在迁移期继续位于 `internal/channelbridge`，也应按
Application Service 管理其生命周期；不把队列、去重或 Turn 状态重新放进
`internal/api.Handler`。跨进程 JSON 类型进入 `internal/apitypes`，内部模型与 Wire DTO
通过显式 Mapper 转换，避免 CLI、extension 或 HTTP Client 依赖内部领域类型。
...
控制面由 Application Bootstrap 统一装配，不由 `cli/serve`、HTTP Handler 或某个
Channel Adapter 临时拼接：

```text
cli/serve
  -> app.Bootstrap
     -> participant / agent services
     -> app/channelwiring: derive binding + select execution mode
     -> app/runtimewiring: register runtime + capabilities
     -> bridge supervisors / native runtime lifecycle
```

Bootstrap 在启动消费者前完成以下校验：

- 每个启用的 Participant 能解析到唯一 Agent 和 Runtime；
- 所选执行模式具备必需 Capability；
- 同一真实 Channel App 不会同时启动 hosted 与 runtime-native 两个入站消费者；
- 必需依赖缺失时启动失败，而不是在第一条消息到达后返回运行期错误；
- shutdown 按 Source、Coordinator、Runtime 的依赖顺序停止，避免遗留 Turn 或 Lease。
...
这里的 `ParticipantBinding` 是从 Participant、Agent 关联关系派生出的只读控制面投影，
不是新的持久化实体。Participant Store 仍是唯一事实源。

Credential 不进入通用 Binding 模型，由 Channel Adapter 通过 Participant Provider
按需解析。Runtime Provisioning 如需生成 OpenClaw/PicoClaw 配置，应由
`app/channelwiring` 将 `ParticipantBinding` 与短生命周期的 Secret Value/Handle 转换为
Runtime 专用输入；不得把 Secret 放入 `Metadata`，也不得再持久化一份包含 Config 和
Credential 的通用 `ChannelBinding`。

两份文档中的 Binding 术语统一如下：

| 名称 | 生命周期 | 用途 |
| --- | --- | --- |
| Participant | 持久化 | Channel 身份与 Agent Binding 的唯一事实源 |
| `ParticipantBinding` | 按需派生 | Execution Planner、Reconciler 和 Channel Adapter 的中立输入 |
| Runtime 专用配置输入 | 单次 Provision/Reconcile | 渲染具体 Runtime 配置，可通过只读 Secret Handle 取值 |
| Event Envelope / DTO | 单次网络传输 | extension 等跨进程调用方使用，不作为 Binding 存储 |
...
接口抽取不能把现有状态简单搬到另一个大对象。状态所有权先固定，再决定第一阶段使用
内存实现还是持久化 Store：

| 状态 | 唯一 Owner | 第一阶段 | 后续持久化条件 |
| --- | --- | --- | --- |
| Participant / Agent / Runtime 绑定 | Participant、Agent Store | 复用现有存储 | 不新增 Bridge Binding 副本 |
| Runtime Session / Turn | Runtime Adapter | 复用 Runtime 状态 | 只持久化恢复确实需要的稳定引用 |
| Conversation / Thread 映射 | BridgeCoordinator Port | 保持现有行为 | 跨重启恢复需求确认后引入 Store |
| Message 去重 / ACK | BridgeCoordinator Port | 进程内、有界缓存 | 需要 at-least-once 处理时引入 Idempotency Store |
| Work Lease | Work Lease Service | 复用现有 API 与状态 | 沿用现有事实源，不在 Coordinator 复制状态机 |

`internal/api` 只解析请求、调用 Use Case 和映射响应；不得持有上述长生命周期状态，
也不得直接读写其持久化文件。
...
Event Envelope 是跨进程 API Contract，目标类型放在 `internal/apitypes`；
`InboundEvent`、`RuntimeEvent` 等内部模型不直接作为 JSON Contract 暴露。SSE/REST
Adapter 负责双向转换，并对未知的 additive 字段保持兼容。

协议兼容采用以下生命周期：

1. 先增加新版本 DTO、Mapper 和 Consumer Contract Test。
2. Producer 在 Capability 协商后发送新版本，迁移期保留旧版本读取。
3. 对旧版本、旧 URL 和兼容 Mapper 记录不含敏感数据的命中统计。
4. 明确最低支持版本、迁移完成标记和删除条件。
5. 只有受支持调用方全部迁移且旧路径在约定窗口内零命中后，才删除兼容代码。

Migration Runner 只负责持久化 Schema 的幂等迁移；协议版本协商由 Adapter 和
Capability Contract 负责，二者不混为同一状态机。
...
本方案可以先于整体后端改造启动，但不能制造与目标分层相反的新债务。与
[后端架构评估](backend-architecture-assessment.zh.md) 的阶段关系如下：

| 本方案阶段 | 对应整体阶段 | 前置与约束 |
| --- | --- | --- |
| 阶段 0 | 整体阶段 0：依赖护栏 | 可立即进行，共用依赖规则和术语 |
| 阶段 1～3 | 整体阶段 1～2：瘦身 API / Bootstrap | 可先抽契约和 Adapter；不得把新状态放入 API 或 `cli/serve`，并预留 Bootstrap 生命周期入口 |
| 阶段 4 | 整体阶段 3：Channel Binding | 在 Application Use Case Owner 明确后收敛所有入口的 Reconcile |
| 阶段 5 | 整体阶段 5：Contract / Migration | 共用 `apitypes`、Migration Runner、命中统计和兼容退出条件 |

Task Core、Registration 等无直接依赖的整体改造不作为本方案前置条件。
...
- 本方案评审完成；
- 术语、责任边界和执行矩阵确认；
- 与后端整体分层对齐，确认 Application、Adapter、Runtime Port 和 API Contract Owner；
- 固定 Binding 与运行状态 Owner，不新增重复事实源；
- Runtime Descriptor 与核心接口定稿；
- Event Envelope V1 定稿；
- 四条关键链路的时序和验收用例确认。
- 建立依赖护栏：对当前 `runtime/agent -> channel/feishu` 依赖使用只减不增的临时
  allowlist，并禁止新增具体 Channel 依赖。
...
- 定义从 Participant 派生 `ParticipantBinding` 的 Application Mapper，并明确 Secret
  Resolver 边界。
- Codex Runtime 实现 `TurnExecutor`。
- `codexmanager` 不再断言具体 `*runtimecodex.Runtime`。
- 现有 CSGClaw、Feishu Codex 链路使用 Adapter 兼容层，行为不变。
- Event Envelope Wire DTO 放入 `apitypes`，与内部事件模型通过 Mapper 转换。
...
- Reconcile 由 Application Use Case 触发，HTTP、CLI 和 Channel 入口复用同一路径。
...
如该阶段修改持久化 Schema，使用版本化、幂等、可审计的 Migration Runner；如删除
旧协议或旧配置兼容路径，必须先满足第 7.2 节定义的命中统计和退出条件。
...
- `internal/runtime`、`internal/agent` 不新增具体 Channel package 依赖，临时 allowlist
  只能收缩。
- `internal/api` 不持有 Bridge 队列、去重、Turn 或协议迁移状态。
- `app.Bootstrap` 在缺失 Capability、Binding 无法解析或入站消费者冲突时稳定失败。
- Application Use Case 可脱离 HTTP、CLI 和具体 Channel SDK 做测试。
- `apitypes` Wire DTO 与内部模型通过 Contract Test 验证转换，不直接类型别名复用。
- 每一阶段运行 `go test ./...`，并对四条核心链路保留针对性回归命令和结果。
...
不要让 Runtime Provisioning、hosted bridge 和 Wire Protocol 各自定义一套可持久化
Channel Binding。三者共同消费从 Participant 派生的 `ParticipantBinding`，再在边界处
转换为短生命周期的 Runtime 配置或 DTO；Contract Test 验证身份字段不发生替代。
...
7. BridgeCoordinator 与 Execution Planner 是否确认为 Application 层职责。
   建议：确认；迁移期可保留 package 路径，但生命周期由 `app.Bootstrap` 管理。
8. `ParticipantBinding` 是否只作为派生投影，Credential 是否统一通过只读 Resolver
   获取。
   建议：确认，避免出现第二套 Binding Store。
9. Event Envelope 是否作为 `apitypes` 中的 Wire DTO，并与内部事件模型分离。
   建议：确认，以便 extension、CLI 和 Server 只依赖稳定协议。
...
- `internal/channelbridge/`：Channel 中立的内部事件模型、数据面 Coordinator 和兼容
  Adapter；迁移期允许保留路径，但不承担 HTTP Contract 或 Bootstrap 职责。
- `internal/channel/csgclaw/`：CSGClaw Channel Source/Sink。
- `internal/channel/feishu/`：Feishu Channel Source/Sink。
- `internal/app/channelwiring/`：派生 ParticipantBinding、Execution Planner、Binding
  Reconcile Use Case、Channel Registry 与 Coordinator 生命周期装配。
- `internal/app/runtimewiring/`：Runtime Capability 注册。
- `internal/apitypes/`：Event Envelope、Capability 等跨进程 Wire DTO。
