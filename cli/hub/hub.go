...
func (c cmd) usage(run *command.Context) {
	run.UsageCommandGroup(c, run.Program+" hub <subcommand> [flags]", []string{
		"list               List hub templates",
		"get <template>     Show one hub template",
		"publish            Publish an existing agent as a hub template",
	})
}

func (c cmd) runList(ctx context.Context, run *command.Context, args []string, globals command.GlobalOptions) error {
	fs := run.NewFlagSet("hub list", run.Program+" hub list [flags]", "List hub templates.")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if len(fs.Args()) != 0 {
		return fmt.Errorf("hub list does not accept positional arguments")
	}
...
func (c cmd) runGet(ctx context.Context, run *command.Context, args []string, globals command.GlobalOptions) error {
	fs := run.NewFlagSet("hub get", run.Program+" hub get <template> [flags]", "Show a hub template.")
	if err := fs.Parse(args); err != nil {
		return err
	}
	rest := fs.Args()
	if len(rest) != 1 {
		return fmt.Errorf("hub get requires exactly one template id")
	}
...
func (c cmd) runPublish(ctx context.Context, run *command.Context, args []string, globals command.GlobalOptions) error {
	fs := run.NewFlagSet("hub publish", run.Program+" hub publish --agent <id> [flags]", "Publish an agent as a hub template.")
	agentID := fs.String("agent", "", "existing agent id to publish")
	registry := fs.String("registry", "", "hub registry to publish into")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if len(fs.Args()) != 0 {
		return fmt.Errorf("hub publish does not accept positional arguments")
	}
	if strings.TrimSpace(*agentID) == "" {
		return fmt.Errorf("hub publish requires --agent")
	}
