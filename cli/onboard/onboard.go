...
func createManagerBot(ctx context.Context, agentsPath, imStatePath string, cfg config.Config, forceRecreateManager bool) (bot.Bot, error) {
	...
	managerImage := cfg.Bootstrap.ManagerImage
	localRef, err := maybeLocalRootfsImageRef(ctx, cfg.Sandbox.Resolved().Provider, managerImage)
	if err != nil {
		return bot.Bot{}, fmt.Errorf("prepare local manager OCI bundle: %w", err)
	}
	if localRef != "" {
		managerImage = localRef
	}
	agentSvc, err := agent.NewServiceWithLLMAndChannels(effectiveLLMConfig(cfg), cfg.Server, cfg.Channels, managerImage, agentsPath, opts...)
	if err != nil {
		return bot.Bot{}, err
	}
	defer func() {
...
func maybeLocalRootfsImageRef(ctx context.Context, sandboxProvider, image string) (string, error) {
	if strings.TrimSpace(sandboxProvider) != config.BoxLiteSDKProvider {
		return "", nil
	}
	image = strings.TrimSpace(image)
	if image == "" || strings.HasPrefix(image, rootfsImagePrefix) {
		return "", nil
	}
	if _, err := exec.LookPath("docker"); err != nil {
		return "", nil
	}
	imageID, err := localDockerImageID(ctx, image)
	if err != nil || strings.TrimSpace(imageID) == "" {
		return "", nil
	}
	cacheRoot, err := managerOCICacheRoot()
	if err != nil {
		return "", err
	}
	key := managerOCICacheKey(image, imageID)
	bundleDir := filepath.Join(cacheRoot, key)
	if isOCIBundleDir(bundleDir) {
		return rootfsImagePrefix + bundleDir, nil
	}
	if err := exportDockerImageToOCIBundle(ctx, image, bundleDir); err != nil {
		return "", err
	}
	return rootfsImagePrefix + bundleDir, nil
}

func localDockerImageID(ctx context.Context, image string) (string, error) {
...
func managerOCICacheRoot() (string, error) {
	root, err := config.DefaultDomainDir("cache")
	if err != nil {
		return "", err
	}
	path := filepath.Join(root, "manager-oci")
...
func managerOCICacheKey(image, imageID string) string {
	sum := sha256.Sum256([]byte(strings.TrimSpace(image) + "\n" + strings.TrimSpace(imageID)))
	// Keep path names readable while maintaining collision resistance.
	return "sha256-" + hex.EncodeToString(sum[:12])
}

func exportDockerImageToOCIBundle(ctx context.Context, image, bundleDir string) error {
	tmpRoot := bundleDir + fmt.Sprintf(".tmp-%d", time.Now().UnixNano())
	...
	tarPath := filepath.Join(tmpRoot, "image.tar")
	if out, err := exec.CommandContext(ctx, "docker", "save", image, "-o", tarPath).CombinedOutput(); err != nil {
		return fmt.Errorf("docker save %q failed: %w (%s)", image, err, strings.TrimSpace(string(out)))
	}

	extracted := filepath.Join(tmpRoot, "bundle")
	if err := os.MkdirAll(extracted, 0o755); err != nil {
		return err
	}
	if out, err := exec.CommandContext(ctx, "tar", "-xf", tarPath, "-C", extracted).CombinedOutput(); err != nil {
		return fmt.Errorf("extract OCI bundle tarball failed: %w (%s)", err, strings.TrimSpace(string(out)))
	}
	if !isOCIBundleDir(extracted) {
		return fmt.Errorf("docker save output is not an OCI layout at %q (requires Docker 25+)", extracted)
	}
	if err := os.RemoveAll(bundleDir); err != nil {
		return err
	}
	if err := os.Rename(extracted, bundleDir); err != nil {
		return err
	}
	return nil
}

func isOCIBundleDir(path string) bool {
