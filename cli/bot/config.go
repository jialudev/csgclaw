...
func (c cmd) runConfig(ctx context.Context, run *command.Context, args []string, globals command.GlobalOptions) error {
	fs := run.NewFlagSet(
		"bot config",
		run.Program+" bot config --channel feishu (--get|--set|--reload) [flags]",
		"Manage bot channel configuration.",
	)
	channelName := fs.String("channel", "feishu", "channel name; only feishu supports bot config")
	...
	if len(fs.Args()) != 0 {
		return fmt.Errorf("bot config does not accept positional arguments")
	}
	if normalizeChannel(*channelName) != "feishu" {
		return fmt.Errorf("bot config currently supports only --channel feishu")
	}
...
func (c cmd) runConfigSet(ctx context.Context, run *command.Context, globals command.GlobalOptions, botID, appID, adminOpenID, secretFile, secretEnv string, secretStdin bool, noReload bool) error {
	...
	if strings.TrimSpace(appID) == "" {
		return fmt.Errorf("bot config --set requires --app-id")
	}
