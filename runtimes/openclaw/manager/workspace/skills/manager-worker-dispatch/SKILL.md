...
description: Use this skill when an admin asks to create, reuse, invite, or dispatch CSGClaw workers/bots, including coding/dev bots for the current room. Use csgclaw-cli to provision workers, add them to rooms, write todo.json, and start manager_worker_api tracking. Do NOT use OpenClaw spawn/subagent for CSGClaw worker creation.
---
...
The CLI is preinstalled in the OpenClaw image at `/usr/local/bin/csgclaw-cli`; do not try to download or install it. If it ever returns `not found`, use the local fallback `python scripts/csgclaw_cli.py` with the same subcommands shown below.
...
## CSGClaw Room Semantics

CSGClaw represents every conversation as a room with an ID. In OpenClaw metadata, a two-person CSGClaw room may arrive as `ChatType: direct`, but it is still a CSGClaw room and can receive worker members.

- If `OriginatingTo`, `To`, or similar metadata is `csgclaw:room:<room_id>`, use `<room_id>` as the current target room.
- If `ConversationLabel` contains a room ID, use it as the target room when no explicit room ID is provided.
- Do not tell the admin there is no group just because `ChatType` is `direct`.
- For "this room", "current room", "current conversation", or "这里/当前这个群", use the current CSGClaw room ID from metadata.
- If no room ID can be resolved from metadata or the admin's message, ask for the room ID once.

## Fast Path

If the admin explicitly asks the manager to arrange or reuse workers such as `ux`, `dev`, and `qa`, or to create/invite a coding/development bot for the current room, do this directly:
...
4. Resolve the target CSGClaw room ID from metadata first; treat `direct` as valid if the target is `csgclaw:room:<room_id>`.
5. Run `csgclaw-cli bot list`, reuse matching available workers, and create a worker only if a required capability is missing or the matching worker is `unavailable`.
6. Ensure every chosen worker has joined the target room and verify this with `csgclaw-cli member list`.
7. For a simple "create a coding/dev bot and add it here" request, stop after worker creation and room membership verification; do not create `todo.json` or start tracking unless there are tasks to dispatch.
8. For task dispatch requests, only after all required workers are present in the room, write `todo.json` under `~/.openclaw/workspace/projects/<slug>/todo.json`.
9. Only after that room-membership check passes, start tracking with `scripts/manager_worker_api.py start-tracking`.
10. Send at most one concise final room reply after creation or tracking starts successfully.
...
4. Choose a suitable project directory under `~/.openclaw/workspace/projects`; create a short slug directory if none fits.
...
```bash
cd ~/.openclaw/csg-skills/manager-worker-dispatch
```

If `csgclaw-cli` is unexpectedly missing, fall back to `python scripts/csgclaw_cli.py` with the same arguments.
...
If you need to direct the human user to the project files on their Mac, point them to the host-side path such as `~/.csgclaw/projects/demo/todo.json`, not the in-box `/home/openclaw/...` path.

## Operating Rules

- Reuse available workers before creating new ones.
- If `csgclaw-cli` exits with `not found` or status `127`, fall back to `python scripts/csgclaw_cli.py` for the same invocation; do not curl/install anything and do not ask the admin what to do.
