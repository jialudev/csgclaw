...
When available, load the CSGClaw API settings from `~/.openclaw/openclaw.json` (or legacy `~/.openclaw/config.json`):

- `channels.csgclaw.baseUrl` or `channels.csgclaw.base_url`
- `channels.csgclaw.accessToken` or `channels.csgclaw.access_token`
