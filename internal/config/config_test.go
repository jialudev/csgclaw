...
func TestSaveWritesModelsSection(t *testing.T) {
	...
	if !strings.Contains(content, "[sandbox]") || !strings.Contains(content, `provider = "boxlite-cli"`) || !strings.Contains(content, `home_dir_name = "sandbox-home"`) {
...
func TestSaveFormatsTopLevelSectionsWithoutExtraWhitespace(t *testing.T) {
	...
	want := `# Generated by csgclaw onboard.
