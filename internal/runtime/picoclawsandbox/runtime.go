...
func (r *Runtime) Kind() string {
	return agentruntime.KindPicoClawSandbox
}
...
func (r *Runtime) GatewayCreateSpec(image, name, botID string, profile agentruntime.Profile) (sandbox.CreateSpec, error) {
	...
	if err := r.deps.EnsureGatewayConfig(name, botID, modelID); err != nil {
	...
	spec := sandbox.CreateSpec{
		...
		AutoRemove: false,
		Env:        envVars,
		Cmd:        []string{"/bin/sh", "-c", GatewayRunCommand()},
	}
	spec.Mounts = append(spec.Mounts,
		sandbox.Mount{HostPath: hostWorkspaceRoot, GuestPath: BoxWorkspaceDir},
		sandbox.Mount{HostPath: projectsRoot, GuestPath: BoxProjectsDir},
	)
	return spec, nil
}

func (r *Runtime) EnsureGatewayConfig(agentName, botID, modelID string) error {
	return r.deps.EnsureGatewayConfig(agentName, botID, modelID)
}

func (r *Runtime) ProjectsGuestPath() string {
	return BoxProjectsDir
}
