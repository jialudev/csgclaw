...
func (s *FeishuService) CreateRoom(req im.CreateRoomRequest) (im.Room, error) {
	...
	if _, ok := s.users[req.CreatorID]; !ok {
		return im.Room{}, fmt.Errorf("creator not found")
	}
	participants, err := s.normalizeParticipantsLocked(req.CreatorID, req.ParticipantIDs)
	if err != nil {
		return im.Room{}, err
	}

	room := im.Room{
		ID:           fmt.Sprintf("fsroom-%d", time.Now().UnixNano()),
		Title:        title,
