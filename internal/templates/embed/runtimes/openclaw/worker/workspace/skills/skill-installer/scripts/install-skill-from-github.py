...
def _prepare_repo(source: Source, method: str, tmp_dir: str) -> str:
    if method in ("download", "auto") and source.owner and source.repo:
        try:
        ...
        raise InstallError("Download mode only supports GitHub repos.")
    if method in ("git", "auto"):
        repo_urls = []
        if source.repo_url:
            repo_urls.append(source.repo_url)
        elif source.owner and source.repo:
            repo_urls.extend(
                [_build_repo_url(source.owner, source.repo), _build_repo_ssh(source.owner, source.repo)]
            )
        else:
            raise InstallError("Missing repo source.")
        last_error = None
...
def _resolve_source(args: Args) -> Source:
    if args.url:
        ...
        return Source(owner=owner, repo=repo, ref=ref, paths=paths, default_name=repo)

    if not args.repo:
        raise InstallError("Provide --repo or --url.")
    if _is_git_repo_url(args.repo):
        ...
        return Source(
            ...
            paths=paths,
            repo_url=args.repo,
            default_name=_derive_repo_name(args.repo),
        )

    repo_parts = [p for p in args.repo.split("/") if p]
    if len(repo_parts) != 2:
        raise InstallError("--repo must be in owner/repo format or be a git clone URL.")
    if not args.path:
    ...
    return Source(
        ...
        ref=args.ref,
        paths=paths,
        default_name=repo_parts[1],
    )


def _skill_name_for_path(source: Source, path: str, explicit_name: str | None, multi_path: bool) -> str:
    if explicit_name and not multi_path:
        return explicit_name
    candidate = os.path.basename(path.rstrip("/"))
    if candidate in ("", "."):
        if source.default_name:
            return source.default_name
        raise InstallError("Unable to derive skill name from repo root path.")
    return candidate
...
def _parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(description="Install a skill from GitHub or a git repo.")
    parser.add_argument("--repo", help="owner/repo or git clone URL")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        try:
            ...
            installed = []
            for path in source.paths:
                skill_name = _skill_name_for_path(
                    source, path, args.name, len(source.paths) > 1
                )
