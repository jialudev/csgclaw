...
安装脚本会从 GitHub Releases 下载预编译 bundle，安装到 `~/.local/lib/csgclaw/<version>/`，并把 `csgclaw` 入口链接到你的 `PATH` 目录中。官方 `csgclaw` bundle 已内置 `boxlite-cli` provider 所需的 `boxlite` 辅助二进制，因此在支持的平台上不需要再单独安装 `boxlite-cli`。目前提供 macOS arm64 和 Linux amd64 的预编译版本。
...
`csgclaw onboard` 会写入本地配置，包含 server、model、bootstrap、sandbox 和 channel 设置。模型 provider 示例、sandbox provider 选项和 Worker 覆盖示例见 [docs/config.zh.md](docs/config.zh.md)。对于官方 release bundle，`boxlite-cli` 默认会直接使用随包分发的 `boxlite`；`boxlite_cli_path` 主要用于高级覆盖和调试场景。
