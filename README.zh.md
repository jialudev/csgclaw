...
安装脚本会下载预编译 release bundle，安装到用户本地目录，并把 `csgclaw` 链接到你的 `PATH`。目前提供 macOS arm64、Linux amd64 和 Linux arm64 的预编译 bundle。
...
- **一键安装** — 提供 macOS arm64、Linux amd64 和 Linux arm64 预编译二进制，几分钟内即可启动
...
**Sandbox** — Worker 执行环境由配置的 sandbox provider 隔离。默认是 **BoxLite**，也支持 `boxlite` 和自定义 provider。
