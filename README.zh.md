...
安装脚本会下载预编译 release bundle，安装到用户本地目录，并把 `csgclaw` 链接到你的 `PATH`。当前 Unix 安装脚本支持 macOS arm64、Linux amd64 和 Linux arm64。

官方 release 也会发布 macOS amd64 和 Windows amd64 bundle。Windows 用户需要从 Releases 下载 `.zip` bundle，解压后从 `csgclaw/bin/` 运行 `csgclaw.exe`。当前 Windows bundle 默认使用 Docker，需要本地可用的 Docker 环境。
