...
import {
  conversationPendingQuestionCount,
  parseAgentActivity,
  parseMessageActivityCommand,
  parsePlainAgentCommand,
  pendingQuestionCount,
  questionOptions,
  threadPendingQuestionCount,
} from "./agentActivity";
...
describe("agent question activity", () => {
  ...
  it("surfaces pending thread questions in room and thread summaries", () => {
    const root: IMMessage = {
      ...
      thread: { latest_reply: questionMessage, reply_count: 1, root_id: "root-1" },
    };

    expect(conversationPendingQuestionCount({ messages: [root] })).toBe(1);
    expect(threadPendingQuestionCount({ root, summary: root.thread })).toBe(1);
  });
});
