...
import { isOpenClawToolDeliveryMessage, parseAgentActivity, parsePlainAgentCommand } from "@/models/agentActivity";
...
export function isToolCallMessage(messageOrContent: IMMessage | unknown): boolean {
  if (isMessageLike(messageOrContent)) {
    return isOpenClawToolDeliveryMessage(messageOrContent) || isNonMessageActivityContent(messageOrContent.content);
  }
