...
function activityEntriesFromRooms(
  ...
  rooms.forEach(({ room, messages }) => {
    ...
    messages.forEach((message) => {
      ...
      const plainCommand = activity
        ? null
        : (parseOpenClawDeliveryCommand(message) ?? parsePlainAgentCommand(message.content));
      ...
      if (activity?.content.msgtype === AgentActivityMsgTypes.tool && activity.content.tool) {
        const toolKey = activityToolMergeKey(activity.content.tool);
        ...
        if (existing?.activity) {
          ...
          if (isTerminalTool(activity.content.tool)) {
        ...
        entries.push(entry);
        if (toolKey && !isTerminalTool(activity.content.tool)) {
...
function activityToolMergeKey(tool: AgentActivityTool): string {
  return firstNonEmpty(tool.id, tool.item_id, tool.tool_call_id);
}

function isTerminalTool(tool: AgentActivityTool): boolean {
  const phase = normalizeFilterValue(tool.phase || "");
  const status = normalizeFilterValue(tool.status || "");
  return (
    phase === "end" ||
    status === "completed" ||
    status === "failed" ||
