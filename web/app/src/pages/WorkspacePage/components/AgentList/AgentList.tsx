...
export function AgentSection({ title, manager, workers, t, activeRoom, busyKey, error, onCreate, onEdit, onStart, onStop, onRecreate, onDelete, onInvite }) {
  ...
  return (
    <div className={`agent-row ${isManager ? "manager" : ""} ${incomplete ? "incomplete" : ""}`.trim()}>
      ...
      <div className="agent-row-main">
        ...
        <div className="agent-meta truncate">{formatProviderLabel(item.provider || item.agent_profile?.provider)} · {agentModelID(item)}</div>
      ...
      <div className="agent-actions">
        ...
        {SHOW_AGENT_LIFECYCLE_ACTIONS && activeRoom && !isManager
          ? (<Button className="agent-action-text" disabled={busyKey.startsWith(busyPrefix)} onClick={() => onInvite(item)}>{t("inviteToRoom")}</Button>)
          : null}
        {!isManager
          ? (
              <Button variant="outlineDanger" className="agent-icon-button danger" aria-label={t("agentDelete")} title={t("agentDelete")} disabled={busyKey.startsWith(busyPrefix)} onClick={() => onDelete(item)}>
