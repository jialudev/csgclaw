...
import { APIKeyField, CLIProxyAuthControl, EnvKeyValueEditor, isBlank, NotifierControls, profileBaseURLMissing, requiredFieldLabel } from "@/components/business/ProfileControls";
import { agentModelID, agentToDraft, formatProviderLabel, formatRuntimeKindLabel, isAgentIncomplete, isAgentRestartNeeded, isAgentRunning, isNotifierRuntimeDraftOnAgentPage, normalizeAuthProviderName, normalizeRuntimeKind, notifierFormIsComplete } from "@/models/agents";
import { AgentIcon } from "@/components/ui/Icons";
import { Button } from "@/components/ui";

export function AgentDetailPane({ item, t, activeRoom, busyKey, error, draft, models, modelBusy, saving, publishBusy, saveError, authStatuses, authBusyProvider, notifierWebhookOrigin, setNotifierWebhookOrigin, onDraftChange, onSave, onPublish, onProviderLogin, onStart, onStop, onRecreate, onDelete, onInvite, onOpenDM }) {
  ...
  const incomplete = draftBelongsToItem && isNotifierRuntimeDraftOnAgentPage(draft, item)
    ...
    <section className="entity-pane agent-detail-pane">
      ...
      <div className="entity-toolbar">
        ...
        {SHOW_AGENT_LIFECYCLE_ACTIONS
          ...
          : null}
        <Button className="preview-action-button" disabled={busyKey.startsWith(busyPrefix)} onClick={() => onOpenDM(item)}>{t("openDM")}</Button>
        {isManager
          ? (<Button variant="outlineDanger" className="preview-action-button preview-action-button-danger" disabled={busyKey.startsWith(busyPrefix) || incomplete} onClick={() => onRecreate(item)}>{t("agentRecreate")}</Button>)
          : SHOW_AGENT_LIFECYCLE_ACTIONS
        ...
        {SHOW_AGENT_LIFECYCLE_ACTIONS && activeRoom && !isManager
          ? (<Button className="preview-action-button" disabled={busyKey.startsWith(busyPrefix)} onClick={() => onInvite(item)}>{t("inviteToRoom")}</Button>)
          : null}
        {!isManager
          ? (<Button variant="outlineDanger" className="preview-action-button preview-action-button-danger" disabled={busyKey.startsWith(busyPrefix)} onClick={() => onDelete(item)}>{t("agentDelete")}</Button>)
          : null}
        {canPublish
          ? (
              <Button
                  ...
                  {!isNotifierRuntimeDraftOnAgentPage(draft, item)
                    ? (
                        <label className="field">
                          ...
                          <input value={draft.image} readOnly disabled onInput={(event) => updateDraft({ image: event.target.value })} placeholder={t("agentImagePlaceholder")} />
                        </label>
                      )
                    : null}
                    ...
                    <section className="profile-section">
                      ...
                      <div className="profile-runtime-grid">
                        <label className="field">
                          ...
                          >
                            {PROVIDERS.map((provider) => (<option key={provider} value={provider}>{formatProviderLabel(provider)}</option>))}
                          </select>
                        </label>
                        <label className="field">
