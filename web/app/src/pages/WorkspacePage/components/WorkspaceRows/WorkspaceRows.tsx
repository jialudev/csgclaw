...
export function WorkspaceGroup({ id, title, count, collapsed, onToggle, onAdd, addLabel, children }) {
  ...
  return (
    <section className={`workspace-group ${collapsed ? "collapsed" : ""}`}>
      ...
      {collapsed ? null : (<div id={itemsID} className="workspace-group-items">{children}</div>)}
    </section>
  );
}
...
export function WorkspaceConversationRow({ conversation, active, currentUserID, usersById, locale, t, onSelect, onPreviewUser }) {
